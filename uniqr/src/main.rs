...
fn run(args: Args) -> Result<()> {
    ...
    match open(&args.in_file) {
        Err(err) => Err(anyhow!("{}: {}", args.in_file, err)),
        Ok(mut file) => {
            ...
            // Need to use closure as the Rust compiler specifically disallows capturing a dynamic value (in this case, args.count) in a func
